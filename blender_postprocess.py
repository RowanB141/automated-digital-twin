...
def clear_scene():
    """Delete the default cube, cameras, and lights from the Blender scene."""
    bpy.ops.object.select_all(action='SELECT')
    bpy.ops.object.delete()


def parse_wld3_offset(wld3_path):
    """Reads the 2nd set of coordinates from the wld3 file to get the Blender offset."""
...
def import_and_position(category_folder):
    """Imports the OBJ file and applies the WLD3 offset."""
...
def import_and_fit_facades():
    """Finds all facade anchoring JSONs, imports them, and fits them using bounding box center."""
    facade_objects = []
    ...
    for anchor_json in anchor_jsons:
        ...
        # 2. Re-origin the mesh to its geometric center
        ...
        # Apply the scale change so thickness measurement is accurate
        ...
        # 4. Rotate mesh based on the specified axis direction
        ...
        # 5. Translate to center and apply the Z-fighting offset (otherwise the facade will sometimes be z-fighting with the building wall)
        ...
        # 6. Apply the final transformation to the object so it is locked in its final global position
        bpy.ops.object.transform_apply(location=True, rotation=True, scale=True)
...
def run_pipeline():
    ...
    print(" -> Snapping 3D Cursor to the center of the Terrain...")
    bpy.ops.object.select_all(action='DESELECT')
